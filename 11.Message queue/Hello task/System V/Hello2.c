...
int main()
{
	...
	{//потомок
		...
		int result = 0;
		int lenght = 0;
		char mymessage[22] = "Hello from Hello2.c!";
		msg_st.mtype = type;
		strncpy(msg_st.message, mymessage, 22); 
		lenght = sizeof(struct msgbuf) - sizeof(long);
		result = msgsnd(mqid2, &msg_st, lenght, 0); //ставим сообщение в очередь
	...
	{//родитель
		...
		int lenght = 0;
		lenght = sizeof(struct msgbuf) - sizeof(long);
		result = msgrcv(mqid1, &msg_st, lenght, type, 0); //пытаемся забрать сообщение из очереди
